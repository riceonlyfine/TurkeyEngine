namespace Turkey.web{
    ...
    export class WebGLDrawCmdManager {
         ...
         * 压入绘制texture指令
         */
        public pushDrawTexture(texture: any, count: number = 2, filter?: Turkey.Filter, textureWidth?: number, textureHeight?: number): void {
            if (filter) {
                //根据filter压入切换program
         ...
         */
        public pushChangeProgram(type: string, filter?: Turkey.Filter): void {
            let key: string;
            ...
            if (type === "texture") {
                ...
                key = (filter as Turkey.CustomFilter).$shaderKey;
                vertSource = (filter as Turkey.CustomFilter).$vertexSrc;
                fragSource = (filter as Turkey.CustomFilter).$fragmentSrc;
