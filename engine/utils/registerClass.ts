namespace Turkey{
     ...
     * 在运行时，这个类的实例将可以使用 Turkey.is() 方法传入一个字符串来判断实例类型。
     * @example 以下代码演示了如何为EventDispatcher类注册运行时类信息并判断类型：
     * <pre>
     *      //为egret.EventDispatcher类注册运行时类信息，由于它实现了IEventDispatcher接口，这里应同时传入接口名对应的字符串。
     *      Turkey.registerClass(Turkey.EventDispatcher,"Turkey.EventDispatcher",["Turkey.IEventDispatcher"]);
     *      let dispatcher = new Turkey.EventDispatcher();
     *      Turkey.log(Turkey.is(dispatcher, "Turkey.IEventDispatcher"));  //true。
     *      Turkey.log(Turkey.is(dispatcher, "Turkey.EventDispatcher"));   //true。
     *      Turkey.log(Turkey.is(dispatcher, "Turkey.Bitmap"));   //false。
     * </pre>
